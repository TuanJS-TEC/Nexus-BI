...
BEGIN TRY
    ...
    /* Add Fact_BanHang rows for 2018-2020 only (idempotent) */
    ;WITH p AS (
        SELECT TOP (300)
            MaMH,
            Gia,
    ...
    t AS (
        ...
        FROM Dim_ThoiGian
        WHERE Nam BETWEEN 2018 AND 2020
    ),
    base AS (
        SELECT
            p.MaMH,
    ...
    base_insert AS (
        SELECT
            ...
            b.BaseKH AS MaKH,
            b.MaTG,
            (ABS(CHECKSUM(b.MaMH, b.MaTG, 'B2018')) % 28) + 4
            ...
            CAST(
                ...
                ) * b.Gia * (1 + ((ABS(CHECKSUM(b.p_idx, b.t_idx, 'B18')) % 13) - 6) / 100.0)
                AS DECIMAL(18, 2)
            ) AS TongTien
        FROM base b
    )
    ...
    extra_insert AS (
        SELECT
            ...
            k.MaKH,
            b.MaTG,
            (ABS(CHECKSUM(b.MaMH, k.MaKH, b.MaTG, 'E2018')) % 24) + 1
            ...
            CAST(
                ...
                ) * b.Gia * (1 + ((ABS(CHECKSUM(k.k_idx, b.p_idx, b.t_idx, 'E18')) % 21) - 10) / 100.0)
                AS DECIMAL(18, 2)
            ) AS TongTien
        FROM base b
        JOIN k
          ON ABS(CHECKSUM(b.MaMH, b.MaTG, k.MaKH, 'E18_FILTER')) % 100 < 8
         AND k.MaKH <> b.BaseKH
    )
    INSERT INTO Fact_BanHang (MaMH, MaKH, MaTG, SoLuongDat, TongTien)
    ...
    /* Add Fact_TonKho rows for 2018-2020 only (idempotent) */
    ;WITH c AS (
        SELECT TOP (40)
            MaCH
        FROM Dim_CuaHang
    ...
    tonkho_insert AS (
        SELECT
            ...
            p.MaMH,
            t.MaTG,
            80 + (ABS(CHECKSUM(c.MaCH, p.MaMH, t.MaTG, 'TK2018')) % 520)
                + CASE WHEN t.Thang IN (1, 2) THEN 50 WHEN t.Thang IN (11, 12) THEN 30 ELSE 0 END AS SoLuongTrongKho
        FROM c
        CROSS JOIN p
        CROSS JOIN t
        WHERE ABS(CHECKSUM(c.MaCH, p.MaMH, t.MaTG, 'TK_FILTER_2018')) % 100 < 42
    )
